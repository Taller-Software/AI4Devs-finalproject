...
class EmailService {
    ...
    public function __construct() {
        try {
            ...
            error_log("⚠️ No se pudo configurar EmailService: " . $e->getMessage());
    ...
    private function configurarMailer(): void {
        try {
            ...
            $this->mailer->Password = Environment::get('SMTP_PASS');
    ...
    public function enviarCodigoLogin(string $email, string $nombre, string $codigo): bool {
        if (!$this->configured) {
            error_log("⚠️ EmailService no configurado. No se puede enviar email a {$email}");
            return false;
        }
        ...
        try {
            ...
            $this->mailer->Subject = 'Código de acceso - Sistema de Herramientas';
            
            // Template HTML del email
            $this->mailer->Body = $this->getLoginCodeTemplate($nombre, $codigo);
            $this->mailer->AltBody = $this->getLoginCodeTextTemplate($nombre, $codigo);
